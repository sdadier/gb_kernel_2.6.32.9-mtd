   ...
   int rtn_value        = 0;
    ...
    else
    {
        /* Reading the MAC Address from .nvmac.info file( the existed file or just created file)*/
    	 rtn_value=kernel_read(fpnv, 0, buf, 18);
        buf[17] ='\n';   // to prevent abnormal string display when mac address is displayed on the screen. 
 	
    	 fp = filp_open(filepath, O_RDONLY, 0);
//        if(IS_ERR(fp) || (fp==NULL))   // If you want to write MAC address to /data/.mac.info once, 
        {
    	     fp = filp_open(filepath, O_RDWR | O_CREAT, 0666);
    	     if(IS_ERR(fp))
    	     {
...
void * dhd_os_prealloc(int section, unsigned long size)
{
	...
	if(nStatic_allocated == 0)	{	    
	    mem_start = (u32)s3c_get_media_memory(S3C_MDEV_WIFI);
		mem_size = (u32)s3c_get_media_memsize(S3C_MDEV_WIFI);
